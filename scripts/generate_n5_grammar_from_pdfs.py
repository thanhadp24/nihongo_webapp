...
GRAMMAR_LESSON_START_ID = 1
GRAMMAR_EXAMPLE_START_ID = 1

PLAN_DIR = Path("plan")
OUTPUT_JSON_PATH = Path("database/import/jlpt_n5_grammar_by_chapter_db_import.json")
OUTPUT_SQL_PATH = Path("database/seed/007_jlpt_n5_grammar.sql")


def find_n5_grammar_folder() -> Path:
    for path in PLAN_DIR.iterdir():
        if path.is_dir() and "n5" in path.name.lower():
            return path
    raise FileNotFoundError("Could not find plan folder containing N5 grammar PDFs")
...
def extract_explanation(title: str, body: list[str]) -> str:
    ...
    return text[:900] if text else f"Mẫu ngữ pháp N5: {title}."
...
def main() -> None:
        ...
        grammar_chapters.append(
            {
                ...
                "description": f"Ngữ pháp N5 - Bài {chapter_number}",
    ...
    output = {
        "metadata": {
            "name": "JLPT N5 grammar import data by chapter",
            "source_folder": str(folder),
            "jlpt_level_id_assumption": JLPT_LEVEL_ID,
            "generated_from": "PDF text layer in plan/ngữ pháp n5",
            "import_order": ["grammar_chapters", "grammar_lessons", "grammar_examples"],
            "notes": [
                "One grammar_chapter is created for each PDF lesson Bài 1-25.",
