...
const level = {
  ...
  jlptLevelId: 2,
  topicStartId: 51,
  sourceUrl: "https://jtest.net/tu-vung-n4"
};

const outputPath = "database/import/jlpt_n4_vocabulary_by_chapter_topic.json";
...
function parseCatalog(html) {
  ...
  const tokenPattern =
    /<a href="\/tu-vung-n4\/chapter-(\d+)"[\s\S]*?<code[^>]*>Chapter\s+\d+<\/code><span[^>]*>([\s\S]*?)<\/span>[\s\S]*?<p class="font-weight-light">([\s\S]*?)<\/p>|<a href="\/tu-vung-n4\/chapter-(\d+)\/section-(\d+)"[\s\S]*?<span class="font-weight-bold">([\s\S]*?)<\/span>[\s\S]*?<small class="font-weight-light">([\s\S]*?)<\/small>/g;
  ...
  for (const match of html.matchAll(tokenPattern)) {
    ...
    if (match[4] && currentChapter) {
      currentChapter.sections.push({
        ...
        source_url: `${sourceBase}/tu-vung-n4/chapter-${match[4]}/section-${match[5]}`
      });
    }
  }
...
const questionHints = ["?", "bao nhiêu", "mấy", "ở đâu", "khi nào", "ai", "gì", "nào", "như thế nào", "tại sao"];
...
function inferPartOfSpeech(word, reading, meaningVi) {
  ...
  if (includesAny(meaning, questionHints)) return "từ nghi vấn";
...
function parseSection(html) {
  const audioMatch = html.match(/<source\s+src="([^"]*\/tango\/sound\/n4\/section\/[^"]+)"/);
...
if (!chapters.length) throw new Error("No N4 chapters parsed");
...
const output = {
  metadata: {
    name: "JLPT N4 vocabulary import data",
