...
const level = {
  ...
  jlptLevelId: 4,
  topicStartId: 147,
  sourceUrl: "https://jtest.net/tu-vung-n2"
};

const outputPath = "database/import/jlpt_n2_vocabulary_by_chapter_topic.json";
...
function parseCatalog(html) {
  ...
  const tokenPattern =
    /<a href="\/tu-vung-n2\/chapter-(\d+)"[\s\S]*?<code[^>]*>Chapter\s+\d+<\/code><span[^>]*>([\s\S]*?)<\/span>[\s\S]*?<p class="font-weight-light">([\s\S]*?)<\/p>|<a href="\/tu-vung-n2\/chapter-(\d+)\/section-(\d+)"[\s\S]*?<span class="font-weight-bold">([\s\S]*?)<\/span>[\s\S]*?<small class="font-weight-light">([\s\S]*?)<\/small>/g;
  ...
  for (const match of html.matchAll(tokenPattern)) {
    ...
    if (match[4] && currentChapter) {
      currentChapter.sections.push({
        ...
        source_url: `${sourceBase}/tu-vung-n2/chapter-${match[4]}/section-${match[5]}`
      });
    }
  }
...
function parseSection(html) {
  const audioMatch = html.match(/<source\s+src="([^"]*\/tango\/sound\/n2\/section\/[^"]+)"/);
...
if (!chapters.length) throw new Error("No N2 chapters parsed");
...
const output = {
  metadata: {
    name: "JLPT N2 vocabulary import data",
