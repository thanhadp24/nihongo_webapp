...
const level = {
  ...
  jlptLevelId: 3,
  topicStartId: 87,
  sourceUrl: "https://jtest.net/tu-vung-n3"
};

const outputPath = "database/import/jlpt_n3_vocabulary_by_chapter_topic.json";
...
function parseCatalog(html) {
  ...
  const tokenPattern =
    /<a href="\/tu-vung-n3\/chapter-(\d+)"[\s\S]*?<code[^>]*>Chapter\s+\d+<\/code><span[^>]*>([\s\S]*?)<\/span>[\s\S]*?<p class="font-weight-light">([\s\S]*?)<\/p>|<a href="\/tu-vung-n3\/chapter-(\d+)\/section-(\d+)"[\s\S]*?<span class="font-weight-bold">([\s\S]*?)<\/span>[\s\S]*?<small class="font-weight-light">([\s\S]*?)<\/small>/g;
  ...
  for (const match of html.matchAll(tokenPattern)) {
    ...
    if (match[4] && currentChapter) {
      currentChapter.sections.push({
        ...
        source_url: `${sourceBase}/tu-vung-n3/chapter-${match[4]}/section-${match[5]}`
      });
    }
  }
...
function parseSection(html) {
  const audioMatch = html.match(/<source\s+src="([^"]*\/tango\/sound\/n3\/section\/[^"]+)"/);
...
if (!chapters.length) throw new Error("No N3 chapters parsed");
...
const output = {
  metadata: {
    name: "JLPT N3 vocabulary import data",
